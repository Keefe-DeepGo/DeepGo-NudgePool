...
describe("short raise - full level", function() {
  ...
  it("Create pool", async function() {
    ...
    const IPdepositAmount = getBigNumber(100000);
    const approveTX = await this.Token1.connect(this.ipAccount1).approve(this.NudgePool.address, IPdepositAmount);
  ...
  it("GP deposit raising", async function() {
    ...
    const gp1DepositAmount = getBigNumber(300);
    const approveGPAccount1TX = await this.Token2.connect(this.gpAccount1).approve(
      this.NudgePool.address, gp1DepositAmount);
    ...
    expect(await this.NudgePoolStatus.getCurGPAmount(
      this.token1, this.token2)).to.be.equal(gp1DepositAmount);
    ...
    expect(GP1BaseTokenAmount).to.be.equal(gp1DepositAmount);
  })
  ...
  it("LP deposit raising", async function() {
    ...
    const lp1DepositAmount = getBigNumber(1000);
    const approveLPAccount1TX = await this.Token2.connect(this.lpAccount1).approve(
      this.NudgePool.address, lp1DepositAmount);
    ...
    expect(await this.NudgePoolStatus.getCurLPAmount(
      this.token1, this.token2)).to.be.equal(lp1DepositAmount);
    ...
    expect(LP1BaseTokenAmount).to.be.equal(lp1DepositAmount);
  })
  ...
  it("GP deposit running", async function() {
    ...
    const gp2DepositAmount = getBigNumber(600);
    const approveGPAccountTX = await this.Token2.connect(this.gpAccount2).approve(
      this.NudgePool.address, gp2DepositAmount);
    ...
    expect(GP2BaseTokenAmount).to.be.equal(gp2DepositAmount);
  });
  ...
  it("LP deposit running", async function() {
    ...
    const lp2DepositAmount = getBigNumber(10000);
    const approveLPAccountTX = await this.Token2.connect(this.lpAccount2).approve(
      this.NudgePool.address, lp2DepositAmount);
